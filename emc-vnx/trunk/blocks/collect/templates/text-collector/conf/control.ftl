<#if vnx??>
	<#list vnx as vnx>
	<@process path="collect-VNX_ID.sh" output="collect-${vnx_index}.sh" encoding="UTF-8" />
	<@process path="fs_dedupe-VNX_ID.sh" output="fs_dedupe-${vnx_index}.sh" encoding="UTF-8" />
	</#list>
</#if>
<@process path="context-block.xml.ftl" />
<@process path="context-file.xml.ftl" />
<@process path="textcollector-process-vnxfile-data.xml.ftl" />
<@process path="textoutputcollector-vnxblock-topology.xml.ftl" />
<@process path="textoutputcollector-vnxblock-performance.xml.ftl" />
<@process path="*.xml" />
<@process path="*.pl" />
<@process path="*.sh" />
<@process path="server_stats" />