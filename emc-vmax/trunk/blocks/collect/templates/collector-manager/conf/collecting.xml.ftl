...
[/#if]
        ...
        <!-- Compute Roll-up Capacity metrics and Storage Pool isfast
            * vhf-vmax-sah.xml
                ...
                * Sum up UsedCapacity where usedby='File' (FileUsedCapacity)
                * Sum up UsedCapacity where usedby='Virtual' (VirtualUsedCapacity)
        ...
        <filter enabled="true" name="VMAX_ICF_PRESCAP" next="VMAX_VHF2" config="Inline-Calculation-Filter/${module['inline-calculation-filter'].instance}/conf/icf-vmax-presentedcap.xml"/>
        
        <!-- Set LunCapacity based on Thick Capacity or Thin ConsumedCapacity metric and isused property -->
        <filter enabled="true" name="VMAX_ICF_USEDCAP" next="VMAX_ICF_PRESCAP" config="Inline-Calculation-Filter/${module['inline-calculation-filter'].instance}/conf/icf-vmax-usedcap.xml"/>
        
        <!-- Cross Reference LUN svclevel with other LUNs -->
        <filter enabled="true" name="VMAX_CRF_SVCLEVEL" next="VMAX_ICF_USEDCAP" config="Cross-Referencing-Filter/${module['cross-referencing-filter'].instance}/conf/crf-vmax-svclevel.xml" />
        ...
        <!-- SMI Performance Filter Chain -->
        
        <!-- Block unwanted SMI performance metrics to avoid flooding DB -->
        <filter enabled="true" name="VMAX_VHF_PERF_FILTER" next="FailOver IgnoreErrors" config="Variable-Handling-Filter/${module['variable-handling-filter'].instance}/conf/vhf-perfbe.xml" />
        
        <!-- Performance metric calculations using SMI raw values -->
        <filter enabled="true" name="VMAX_ICF_PERF_SMICALC" next="VMAX_VHF_PERF_FILTER" config="Inline-Calculation-Filter/${module['inline-calculation-filter'].instance}/conf/icf-vmax-perf-smicalc.xml"/>
        
        <!-- Performance metric calculations using SMI raw values and properties -->
        <filter enabled="true" name="VMAX_VHF_PERF_SMICALC" next="VMAX_ICF_PERF_SMICALC" config="Variable-Handling-Filter/${module['variable-handling-filter'].instance}/conf/vhf-smiperf.xml" />
    </filters>
    ...
    <collectors>
        ...
        [#if vmax??]
            ...
            [/#list]
        [/#if]
        <collector enabled="${enabled}" name="VMAX-TOPO-SE" next="VMAX_PTF_LUN_DGSTYPE" config="XML-Collector/${module['xml-collector'].instance}/conf/xmlcollector-vmax-topo.xml" />
        [#if smi??]
        <collector enabled="${enabled}" name="VMAX-PERF-SMI" next="VMAX_VHF_PERF_SMICALC" config="XML-Collector/${module['xml-collector'].instance}/conf/xmlcollector-vmax-perf-smi.xml" />
        [/#if]
        [#if emcvmax??]
        <collector enabled="true" name="VMAX-PERF-UNIVMAX" next="VMAX_PTF_LUN_DGSTYPE" config="XML-Collector/${module['xml-collector'].instance}/conf/xmlcollector-vmax-perf.xml" />
        [/#if]
        <collector enabled="[#if use_topo]true[#else]false[/#if]" name="XML-API-Block-Sparql" next="Backend" config="XML-Collector/${module['xml-collector'].instance}/conf/vmax-sparql-collector.xml" />
    </collectors>
</config>