...
[/#if]
		...
		<filter enabled="true" name="VMAX_CRF1" next="VMAX_PTF1" config="Cross-Referencing-Filter/${module['cross-referencing-filter'].instance}/conf/crf-vmax1.xml" />
	</filters>
	<collectors>
		<collector enabled="true" name="vmax-topo" next="VMAX_CRF1" config="XML-Collector/${module['xml-collector'].instance}/conf/xmlcollector-vmax-topo.xml" />
		[#if emcvmax??]
		<collector enabled="true" name="vmax-perf" next="VMAX_CRF1" config="XML-Collector/${module['xml-collector'].instance}/conf/xmlcollector-vmax-perf.xml" />
		[/#if]
	</collectors>
</config>