...
<xsl:stylesheet version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns="http://www.emc.com/em/2012/07/univmax/restapi/performance" xmlns:ns2="http://www.emc.com/em/2012/07/univmax/restapi/common" xmlns:ns3="http://www.emc.com/em/2012/07/univmax/restapi/mobile">
	...
	<xsl:variable name="devicegroup" select="$request//ns2:deviceGroupId"/>
	<xsl:variable name="filePath" select="'tmp/devicegrouptmp2/'"/>
	<xsl:variable name="outputPath" select="'tmp/devicegroupout/'"/>
	<xsl:variable name="tempFilename" select="concat ( $filePath , $device, $devicegroup , '.metrics.xml' )"/>
	<xsl:variable name="outputFilename" select="concat ( $outputPath , $device, $devicegroup , '.out.xml' )"/>
	...
	<xsl:template match="/">
		<xsl:result-document href="{$outputFilename}" format="xml">
			<w4n>
				...
				<xsl:copy-of select="$devicegroup" copy-namespaces="no"/>
